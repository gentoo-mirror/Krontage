...
# Shell module https://github.com/agentzh/echo-nginx-module/tags
HTTP_ECHO_MODULE_PV="0.41"
HTTP_ECHO_MODULE_PN="ngx-shell"
...
# Redis module by http://people.freebsd.org/
HTTP_REDIS_MODULE_PV="0.3.6"
HTTP_REDIS_MODULE_PN="ngx-redis"
HTTP_REDIS_MODULE_P="${HTTP_REDIS_MODULE_PN}-${HTTP_REDIS_MODULE_PV}"
HTTP_REDIS_MODULE_URI="http://people.freebsd.org/~osa/ngx_http_redis-${HTTP_REDIS_MODULE_PV}.tar.gz"

# Redis2 module by http://openresty.org
HTTP_REDIS2_MODULE_PV="1.2.1.14"
HTTP_REDIS2_MODULE_PN="ngx-redis2"
HTTP_REDIS2_MODULE_P="${HTTP_REDIS2_MODULE_PN}-${HTTP_REDIS2_MODULE_PV}"
HTTP_REDIS2_MODULE_URI="http://agentzh.org/misc/nginx/ngx_openresty-${HTTP_REDIS2_MODULE_PV}.tar.gz"
...
CDEPEND="
	...
	nginx_modules_http_redis2? ( dev-db/redis )"
RDEPEND="${CDEPEND}"
DEPEND="${CDEPEND}
...
src_configure() {
	...
	if use nginx_modules_http_redis; then
		http_enabled=1
		myconf+=" --add-module=${WORKDIR}/ngx_http_redis-${HTTP_REDIS_MODULE_PV}"
	fi

	if use nginx_modules_http_redis2; then
		http_enabled=1
		myconf+=" --add-module=${WORKDIR}/ngx_openresty-${HTTP_REDIS2_MODULE_PV}/bundle/redis2-nginx-module-0.08rc4/"
	fi
