...
src_prepare() {
	...
	if use dlz; then
		...
		if use sdb-ldap; then
			epatch "${FILESDIR}"/sdb-ldap_patch-1.patch
			epatch "${FILESDIR}"/sdb-ldap_patch-2.patch
			cp -fp ${FILESDIR}/ldapdb.c bin/named/
			cp -fp ${FILESDIR}/ldapdb.h bin/named/include/

			cp -fp ${FILESDIR}/ldap2zone.c bin/tools/
			cp -fp ${FILESDIR}/{zone2ldap.1,zone2ldap.c} bin/tools/
		fi
	fi
